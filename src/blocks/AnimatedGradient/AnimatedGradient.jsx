// Fondo de gradiente animado (WebGL2). Adaptado a JSX plano para este proyecto
// (sin Tailwind/TypeScript): mismo shader, con fallback CSS si WebGL no está disponible.
...
const presets = {
  Aurora: {
    ...
    distortion: 40, swirl: 80, swirlIterations: 10,
    softness: 100, offset: 200, shape: 'Edge', shapeSize: 50,
  },
  Oceanic: {
    color1: '#000814', color2: '#001d3d', color3: '#00b4d8',
    rotation: 0, proportion: 70, scale: 0.4, speed: 10,
    distortion: 15, swirl: 50, swirlIterations: 12,
    softness: 80, offset: 150, shape: 'Checks', shapeSize: 30,
  },
  Amber: {
    color1: '#140c00', color2: '#4a2500', color3: '#f57c00',
    rotation: 120, proportion: 80, scale: 0.8, speed: 20,
    distortion: 25, swirl: 60, swirlIterations: 8,
    softness: 90, offset: 500, shape: 'Stripes', shapeSize: 40,
  },
  Toxic: {
    color1: '#050d05', color2: '#0a240a', color3: '#39ff14',
    rotation: -90, proportion: 55, scale: 0.5, speed: 25,
    distortion: 60, swirl: 100, swirlIterations: 15,
    softness: 70, offset: -100, shape: 'Edge', shapeSize: 20,
  },
  Ghost: {
    color1: '#0a0a0a', color2: '#1c1c1c', color3: '#a3a3a3',
    rotation: 45, proportion: 50, scale: 0.3, speed: 8,
    distortion: 10, swirl: 30, swirlIterations: 5,
    softness: 100, offset: 0, shape: 'Checks', shapeSize: 60,
  },
}

export function AnimatedGradient({ config = { preset: 'Aurora' }, noise, radius = '0px', style, className = '' }) {
  ...
  // fallback sin WebGL: degradado estático con los mismos colores
  if (hasWebGLError) {
    return (
      <div
        className={`ag-container ${className}`}
        style={{
          borderRadius: radius,
          background: `linear-gradient(135deg, ${params.color1}, ${params.color2} 55%, ${params.color3})`,
          ...style,
        }}
      />
    )
  ...
  return (
    <div ref={containerRef} className={`ag-container ${className}`} style={{ borderRadius: radius, ...style }}>
      ...
      {noise && noise.opacity > 0 && (
        <div
          style={{
            ...
            inset: 0,
            backgroundImage:
              'url("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAADAAAAAwBAMAAAClLOS0AAAAElMTVEUAAAAAAAAAAAAAAAAAAAAAAADgKxmiAAAABnRSTlMCCgkGBAVJOAVJAAAASklEQVQ4y2NgGAWjYBSMglEwCgY/YGRgZBQUYmJiZGQEkYwMjIyMgoKCjIyMIJKBgRFIMjIyAklGRkYGRkFBYEcwMDIyMjAOUQAA1I4HwVwZAkYAAAAASUVORK5CYII=")',
